...
// Settings page (quota limits editor)
...
//
// Read/write the `limits` block from server.config.  Writes update the in-memory
// config immediately (so the running messageHandlers pick up new quotas on the
// next request) and persist to server.config.local so the change survives a
// restart.  server.config.local is git-ignored and merged on top of
// server.config by the config loader (shallow merge — the whole `limits`
// object is replaced, so we always write all three keys).
...
function getLocalConfigPath() {
    return path.join(__dirname, '..', '..', global.m_serverconfig.getFileName() + '.local');
}

// Read server.config.local if it exists, returning a parsed object (comments
// stripped).  Returns {} if the file does not exist.
function readLocalConfig() {
    ...
    try {
        ...
        console.error('[dashboard] Could not parse server.config.local: ' + e.message);
        return {};
    }
}

// Write server.config.local with the updated `limits` block.  Preserves any
// other keys already in the file.  Written as clean JSON with a header
// comment explaining it is machine-generated.
function writeLocalConfigLimits(limits) {
    const localPath = getLocalConfigPath();
    const existing = readLocalConfig();
    existing.limits = limits;
    const header = '// Auto-generated by storage server admin dashboard.\n' +
        '// Merged on top of server.config at startup (shallow merge).\n' +
        '// Do NOT commit — this file is git-ignored.\n';
    fs.writeFileSync(localPath, header + JSON.stringify(existing, null, 4) + '\n', 'utf8');
}

// API: get current quota limits
router.get('/api/settings', requireAuth, (req, res) => {
    try {
        ...
        res.json({
            error: 0,
            limits: {
                max_news_per_account: limits.max_news_per_account ?? 0,
                max_missions_per_account: limits.max_missions_per_account ?? 0,
                max_missions_per_unit: limits.max_missions_per_unit ?? 0
            },
            persisted: fs.existsSync(getLocalConfigPath())
        });
    } catch (error) {
        console.error('Error fetching settings:', error);
        res.json({ error: 1, errorMessage: 'Failed to fetch settings' });
    }
});

// API: update quota limits (in-memory + persist to server.config.local)
router.post('/api/settings', requireAuth, (req, res) => {
    try {
        const { max_news_per_account, max_missions_per_account, max_missions_per_unit } = req.body;

        // Validate: each must be a non-negative integer (0 = unbounded)
        const parse = (val) => {
        ...
        const news = parse(max_news_per_account);
        const missions = parse(max_missions_per_account);
        const perUnit = parse(max_missions_per_unit);

        if (news === null || missions === null || perUnit === null) {
            return res.json({ error: 1, errorMessage: 'All limits must be non-negative integers (0 = unbounded)' });
        }

        const limits = {
            max_news_per_account: news,
            max_missions_per_account: missions,
            max_missions_per_unit: perUnit
        };

        // Update in-memory config (immediate effect for running messageHandlers)
        global.m_serverconfig.m_configuration.limits = limits;

        // Persist to server.config.local (survives restart)
        writeLocalConfigLimits(limits);

        console.log(`[dashboard] quota limits updated by ${req.session.adminUsername}: ` +
            `news=${news}, missions=${missions}, perUnit=${perUnit}`);

        res.json({ error: 0, limits: limits });
    } catch (error) {
        console.error('Error saving settings:', error);
        res.json({ error: 1, errorMessage: 'Failed to save settings' });
    }
});
